...
cfolder = 'output_EAS_w_prediction';
mkdir(cfolder);   
...
for jj = 1:length(ggg)
   ...
   load(['output_prediction_result/','prediction_',ggg(jj).name(1:end)]);
    ...
    %% EAS method of artifact removal
    ...
    for i = 2:len-1
        for j = 1:5
            ...
            elseif pred(i,j) == 0
                estimated_artifact((20*t+st)*sampling_rate+1:(20*t+st+4)*sampling_rate) = 0;
    ...
    %%
    f1 = [cfolder,'/',ggg(jj).name(1:end-4),'_EAS_with_prediction.mat'];
