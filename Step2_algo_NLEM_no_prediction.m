...
addpath('TF_anaylsis');
addpath('TF_anaylsis/tool');
%foldername = 'preprocessed_signal_4sec_seg';
foldername = 'database';
...
cfolder = 'output_NLEM_no_prediction'; 
mkdir(cfolder);     
...
for jj = 1:length(ggg)
    ...
    %% NLEM
    [all_pk] = NLEM_new(EEG_pd2,locEEG_pd2, 50, sampling_rate, 5, 0.65 ,1, 1);
    ...
    % TODO: if two consecutive beats are less than 100 points
    ...
    locEEG = beats2;
    %%
    f1 = [cfolder,'/',ggg(jj).name(1:10),'_NLEM_new_no_prediction.mat'];
